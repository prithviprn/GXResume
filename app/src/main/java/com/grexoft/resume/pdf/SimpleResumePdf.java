...
public class SimpleResumePdf extends ResumePdf {

	public SimpleResumePdf(Context context, String templateName) {
	...
	protected void setTitleOfDocument() throws DocumentException {
		try {
			...
			String resumeTitle = (Common_Utilty
					.isNotEmptyString(resumeData.title) && resumeData.showTitle) ? "\n"
					+ resumeData.title : "";
			...
			if (Common_Utilty.isNotEmptyString(resumeData.image) && resumeData.showImage) {
				...
				cellTextOrImage.setPaddingBottom(15);
		...
		if (blockName.equals(OBJECTIVE_HEADING)) {
			...
			for (int i = 0; i < resumeData.educationDetail.size(); i++) {
				...
				if (i < resumeData.educationDetail.size() - 1) {
					educationParagraph.setSpacingAfter(4);
				}
